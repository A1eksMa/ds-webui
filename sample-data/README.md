...
| Файл | Что |
|---|---|
| `CRM.js` | `window.DS.sources["CRM"]` — 54 объекта, показатели `email` / `phone` / `status` |
| `ERP.js` | `window.DS.sources["ERP"]` — 53 объекта, показатели `price` / `stock`, ключ `client_ref` |
...
- **Устаревание** — в `manifest.js` у `CRM` `db_max_cnt` > `gen_max_cnt`: после генерации
  файла в базу пришла ещё одна CRM-транзакция (клиент 105) → источник помечается
  «пересобрать». У `ERP` они равны — свежий.
- **Клиенты 106-155** — полсотни обычных активных клиентов (полностью matched), без
  собственной истории/edge-case — только чтобы в таблице было заметно больше строк
  (~2-3 экрана), удобно тестировать скролл и sticky-шапку на «Таблице».
